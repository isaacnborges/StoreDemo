...
namespace NerdStore.Catalogo.Domain.EventHandlers
{
    public class PedidoIniciadoEventHandler : INotificationHandler<PedidoIniciadoEvent>
    {
        ...
        public async Task Handle(PedidoIniciadoEvent notification, CancellationToken cancellationToken)
        {
            ...
            if (result)
            {
                await _mediatorHandler.PublicarEvento(new PedidoEstoqueConfirmadoEvent(notification.PedidoId, notification.ClienteId, notification.Total, 
                    notification.NomeCartao, notification.NumeroCartao, notification.ExpiracaoCartao, notification.CvvCartao, notification.ProdutosPedido));
            }
            else
            {
